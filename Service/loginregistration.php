...
switch ($action) {
    ...
    case "gettoken": $username = filter_input(INPUT_POST, "username");
        ...
        // temp solution, should be send via email;
        echo "reset.php?username=$username&token=$returns";
        exit();
    case "resetpassword": $username = filter_input(INPUT_POST, "username");
        $userToken = filter_input(INPUT_POST, "token");
        $newPassword = filter_input(INPUT_POST, "newPassword");
        $returns = C_LoginRegistration::resetPassword($username, $userToken, true, $newPassword);
        if($returns) {
            header("Location: /index.php");
            exit();
        }
