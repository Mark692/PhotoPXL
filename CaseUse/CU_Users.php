...
class CU_Users
{
     ...
     */
    public function manage_profilePIC($username, $photo_ID, $path)
    {
        $this->get_ProfilePic($username);

        $this->set_ProfilePic($username, $photo_ID);

        $this->upload_NewCover($username, $path);
...
//In questo caso si potrebbe usare la funzione \Foundation\F_User::get_UserDetails($username); e mostrarne l'output
     ...
     *
     * @param string $username L'utente che vuole cambiare la sua foto profilo
     * @param string $path Il percorso dal quale prendere la nuova foto da caricare
     * @return boolean Indica l'esito delle funzioni. TRUE = nessun errore, FALSE = almeno uno
     */
    public function upload_NewCover($username, $path)
    {
    ...
    public function set_PhotoPrivacy($username, $photo_ID, $privacy)
    {
        try
        {
            ...
            echo("Query effettuata. ".nl2br("\r\n")."L'esito di questa funzione va verificato nel DB. ".nl2br("\r\n"));
            return TRUE;
        }
        catch(queries $q2)
        {
                ...
                try
                {
                    ...
                    if($role !== FALSE)
                    {
                        ...
                        foreach($ruoli_Utenti as $nome_Ruolo => $valore_Ruolo)
                        {
                            if($valore_Ruolo == $role)
                            {
                                echo("L'utente $username ha ora ruolo: ".$nome_Ruolo);
                                return TRUE;
                            }
                        }
                    }
